...
class CuBLASLinearFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input, weight, bias):
        output = torch.zeros(input.size(0), weight.size(0), device=input.device)
        linear_kernel.linear_forward_cublas(input, weight, bias, output)
    ...
    def backward(ctx, grad_output):
        ...
        linear_kernel.linear_backward_cublas(grad_output, input, weight, grad_input, grad_weight, grad_bias)
...
class CuBLASLinear(nn.Module):
    ...
    def forward(self, x):
        return CuBLASLinearFunction.apply(x, self.weight, self.bias)